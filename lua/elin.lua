local version = "0.1.1"
local verbose = false
local cache_rtp = (vim.fn.stdpath("cache") .. "/elin-rtp")
local function compile_nvim()
  if vim.uv.fs_stat(cache_rtp, nil) then
    local _let_1_ = require("elin.api")
    local compile_nvim_config = _let_1_["compile-nvim-config"]
    local elin_rtp = (vim.fn.stdpath("cache") .. "/elin-rtp")
    vim.uv.fs_rename(elin_rtp, (elin_rtp .. "-bak"), nil)
...
local function use_elin_rtp()
  if not vim.o.runtimepath:match("/elin-rtp") then
    return vim.o.runtimepath:append(cache_rtp)
  else
    return nil
  end
end
